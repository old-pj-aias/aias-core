...
impl Signer {
    pub fn new(
        ...
        let judge_pubkey = RSAPublicKey::from_pkcs8(&judge_pubkey.contents)
            .expect("failed to parse judge public keypkcs8");
    ...
    pub fn new_with_blinded_digest(
        signer_privkey: String,
        signer_pubkey: String,
        digest_and_ej: String,
        id: u32,
    ) -> Self {
        let (signer_privkey, signer_pubkey) = Self::parse_keys(signer_privkey, signer_pubkey);
    ...
    fn parse_keys(privkey: String, pubkey: String) -> (RSAPrivateKey, RSAPublicKey) {
        ...
        let privkey = RSAPrivateKey::from_pkcs1(&privkey.contents)
            .expect("failed to parse signer private key pkcs1");

        let pubkey = pem::parse(pubkey).expect("failed to parse signer public key pem");
        let pubkey = RSAPublicKey::from_pkcs8(&pubkey.contents)
            .expect("failed to parse signer public key pkcs8");
